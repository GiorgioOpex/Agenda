  ...
  return(<div style={{position:"relative"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3}}>{cells.map(function(d,i){
      ...
      return(<div key={key}
        ...
        {has&&!we&&!isDragSrc&&<div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column"}}><div style={{flex:1,background:amBg,opacity:.88,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>{en&&en.am&&en.am.status==="client"&&en.am.client&&<span style={{fontSize:6,fontWeight:600,color:"#fff",textShadow:"0 1px 1px rgba(0,0,0,.5)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:"100%",padding:"0 1px"}}>{en.am.client}</span>}</div><div style={{flex:1,background:pmBg,opacity:.88,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>{en&&en.pm&&en.pm.status==="client"&&en.pm.client&&<span style={{fontSize:6,fontWeight:600,color:"#fff",textShadow:"0 1px 1px rgba(0,0,0,.5)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:"100%",padding:"0 1px"}}>{en.pm.client}</span>}</div></div>}
