...
export function Calendar(p){
  ...
  function handleCellClick(key,en,e){
    if(dragInfo){return;}
    var we=false;var p2=key.split("-").map(Number);var dd=p2[2];we=(fd+dd-1)%7===6;
    if(we)return;
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3}}>{cells.map(function(d,i){
      ...
      return(<div key={key}
        ...
        style={{position:"relative",aspectRatio:"1",display:"flex",alignItems:"center",justifyContent:"center",borderRadius:8,overflow:"hidden",
          cursor:dragInfo?(canBeTarget?"pointer":isDragSrc?"default":"not-allowed"):we?"default":"pointer",
          border:isDragTgt?"2px dashed #F0C040":isT?"2px solid "+CL.red:"1px solid #e8e8e8",
          background:isDragSrc?"#FFE0B2":isDragTgt?"#FFF8E1":we?"#f0f0f0":canBeTarget?"#FFFDE7":isBlocked?"#f8f8f8":isSat?"#f7f5f0":"#fafafa",
          transition:"all .15s",userSelect:"none",opacity:isDragSrc?0.5:isBlocked?0.4:1}}>
        {has&&!we&&!isDragSrc&&<div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column"}}><div style={{flex:1,background:amBg,opacity:.88,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>{en&&en.am&&en.am.status==="client"&&en.am.client&&<span style={{fontSize:6,fontWeight:600,color:"#fff",textShadow:"0 1px 1px rgba(0,0,0,.5)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:"100%",padding:"0 1px"}}>{en.am.client.toUpperCase()}</span>}{en&&en.am&&en.am.status==="commercial"&&<span style={{fontSize:5,fontWeight:700,color:"#fff",textShadow:"0 1px 1px rgba(0,0,0,.5)",letterSpacing:.3}}>COMMERCIALE</span>}</div><div style={{flex:1,background:pmBg,opacity:.88,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>{en&&en.pm&&en.pm.status==="client"&&en.pm.client&&<span style={{fontSize:6,fontWeight:600,color:"#fff",textShadow:"0 1px 1px rgba(0,0,0,.5)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:"100%",padding:"0 1px"}}>{en.pm.client.toUpperCase()}</span>}{en&&en.pm&&en.pm.status==="commercial"&&<span style={{fontSize:5,fontWeight:700,color:"#fff",textShadow:"0 1px 1px rgba(0,0,0,.5)",letterSpacing:.3}}>COMMERCIALE</span>}</div></div>}
