...
export function Consuntivo(p){
  ...
  return(<div style={{overflowX:"auto"}}>
    <table style={{borderCollapse:"collapse",width:"100%",fontSize:13,fontFamily:FONT}}><thead><tr style={{background:"#FFF8F8"}}>
      ...
      <th style={{padding:"10px 8px",borderBottom:"2px solid "+CL.red,color:"#1565C0"}}>Comm.</th></tr></thead>
    <tbody>{cons.map(function(n){var r=report[n];return(<tr key={n}><td style={{padding:"8px 14px",borderBottom:"1px solid #eee",fontWeight:600,color:CL.greyDk,textAlign:"left"}}>{n}</td>
      ...
      <td style={{padding:"8px",borderBottom:"1px solid #eee",textAlign:"center",color:"#1565C0",fontWeight:600}}>{r.tcom?fmtNum(r.tcom):"-"}</td></tr>);})}</tbody></table>
    ...
    <p style={{marginTop:12,fontSize:11,color:"#aaa"}}>Valori in giornate (0.5 = mezza giornata). Sabati impegnati inclusi nel conteggio clienti.</p></div>);
}
