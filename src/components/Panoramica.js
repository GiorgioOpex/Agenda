...
import { CL, FONT, makeKey, daysInMonth, firstDow, fmtNum, getHalfBg, getClientColor, getUsedClients, calcMonthlyPlanned, isHoliday, HOLIDAY_BG, HOLIDAY_LETTER } from "./shared";
  ...
  return(<div>
    ...
    <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:14,alignItems:"center"}}>
      ...
      {hasTrain&&<div onClick={function(){sFilter(filter==="__training"?null:"__training");}} style={{display:"flex",alignItems:"center",gap:4,cursor:"pointer",padding:"3px 8px",borderRadius:6,border:filter==="__training"?"2px solid #7B1FA2":"2px solid transparent",background:filter==="__training"?"#fff":"transparent"}}>
        <div style={{width:13,height:13,borderRadius:4,background:"#7B1FA2"}}/><span style={{fontSize:11,color:CL.greyMd}}>Formazione API</span></div>}
    </div>
    ...
    <thead><tr>
      ...
      {Array.from({length:days},function(_,i){var isSun=(fd+i)%7===6;var isSat=(fd+i)%7===5;var isHol=isHoliday(year,month,i+1,customHolidays);return<th key={i+1} style={{padding:"5px 2px",borderBottom:"2px solid "+CL.red,textAlign:"center",minWidth:24,color:isSun||isHol?"#bbb":isSat?"#bba":CL.greyMd,fontSize:10}}>{i+1}</th>;})}
    ...
    <tbody>{consSorted.map(function(name){var cE=entries[name]||{},totCli=0,totBusy=0,totComm=0,totTrain=0;
      var tds=Array.from({length:days},function(_,i){var key=makeKey(year,month,i+1),e=cE[key],isSun=(fd+i)%7===6,isSat=(fd+i)%7===5;
        ...
        // Cella festiva: nera con F, occupa entrambe le metà
        if(isHol){return<td key={i+1} style={{padding:1,borderBottom:"1px solid #eee",textAlign:"center"}}><div style={{width:18,height:18,borderRadius:3,margin:"0 auto",background:HOLIDAY_BG,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:9,fontWeight:700,fontFamily:FONT}}>{HOLIDAY_LETTER}</div></td>;}
