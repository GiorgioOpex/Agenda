...
export function DayModal(p){
  var dk=p.dk,entry=p.entry,clients=p.clients,onSave=p.onSave,onMove=p.onMove,onClose=p.onClose;
  ...
  var mv=useState(false),showMove=mv[0],sShowMove=mv[1];
  var nd=useState(""),newDate=nd[0],sNewDate=nd[1];
  var info=parseKey(dk);

  var hasImpegno=entry&&((entry.am&&(entry.am.status==="client"||entry.am.status==="busy"))||(entry.pm&&(entry.pm.status==="client"||entry.pm.status==="busy")));
  ...
  async function doMove(){
    if(!newDate){alert("Seleziona una data di destinazione");return;}
    if(newDate===dk){alert("La data di destinazione e' uguale a quella attuale");return;}
    var destDate=new Date(newDate);
    var dow=destDate.getDay();
    if(dow===0||dow===6){alert("Non puoi spostare su un weekend");return;}
    sSv(true);
    await onMove(dk,newDate,entry);
    sSv(false);
  }
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.45)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000}} onClick={onClose}>
    <div onClick={function(e){e.stopPropagation();}} style={{background:"#fff",borderRadius:16,padding:28,width:400,maxWidth:"92vw",maxHeight:"88vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,.2)",fontFamily:FONT}}>
      ...
      {hasImpegno&&!showMove&&<div style={{marginTop:12,paddingTop:12,borderTop:"1px solid #eee"}}>
        <button onClick={function(){sShowMove(true);}} style={{width:"100%",padding:"9px 0",borderRadius:8,border:"1px solid "+CL.grey,background:"#fff",color:CL.grey,fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:FONT}}>Sposta a un altro giorno</button>
      </div>}

      {showMove&&<div style={{marginTop:12,padding:14,background:"#FFF8F0",borderRadius:10,border:"1px solid #F0C040"}}>
        <div style={{fontSize:13,fontWeight:600,color:CL.greyDk,marginBottom:8}}>Sposta impegno</div>
        <div style={{fontSize:12,color:CL.greyMd,marginBottom:10}}>L'impegno del {info.day} {MESI[info.month]} viene spostato alla nuova data. Il giorno corrente diventa libero.</div>
        <input type="date" value={newDate} onChange={function(e){sNewDate(e.target.value);}} style={Object.assign({},sI,{width:"100%",padding:"10px 12px",fontSize:14,border:"2px solid #F0C040",marginBottom:10})}/>
        <div style={{display:"flex",gap:8}}>
          <button onClick={doMove} disabled={saving||!newDate} style={Object.assign({},sB,{flex:1,padding:"9px 0",background:"#E68A00",opacity:saving||!newDate?0.5:1})}>{saving?"Spostamento...":"Conferma spostamento"}</button>
          <button onClick={function(){sShowMove(false);sNewDate("");}} style={Object.assign({},sO,{padding:"9px 12px",fontSize:12})}>Annulla</button>
        </div>
      </div>}
