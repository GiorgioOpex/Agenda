...
function HalfEditor(p){
  var opts=[{k:"",l:"Libero"},{k:"client",l:"Cliente OPEX"},{k:"commercial",l:"Commerciale OPEX"},{k:"training",l:"Formazione API"},{k:"busy",l:"Altro impegno"}];
  return(<div style={{marginBottom:12}}>
    ...
    {p.status==="client"&&p.client&&p.expiredClient===p.client&&<div style={{padding:"6px 10px",background:"#FFF3F3",borderRadius:6,border:"1px solid #e53935",fontSize:12,color:"#e53935",marginBottom:6}}>Contratto scaduto per questo cliente!</div>}
...
export function DayModal(p){
  var dk=p.dk,entry=p.entry,clients=p.clients,clientEndDates=p.clientEndDates||{},onSave=p.onSave,onClose=p.onClose;
  ...
  function isExpired(clientName){
    if(!clientName||!clientEndDates[clientName])return false;
    var endDate=new Date(clientEndDates[clientName]);
    var dayDate=new Date(dk);
    return dayDate>endDate;
  }

  function expiredMsg(clientName){
    if(!clientName)return null;
    var ed=clientEndDates[clientName];
    if(!ed)return null;
    return "Contratto "+clientName+" scade il "+ed;
  }

  async function doSave(){
    if(as==="client"&&!ac){alert("Seleziona un cliente (mattina)");return;}
    if(ps==="client"&&!pc){alert("Seleziona un cliente (pomeriggio)");return;}
    if(as==="client"&&ac&&isExpired(ac)){alert("Non puoi inserire "+ac+" in questa data: il contratto e' scaduto il "+clientEndDates[ac]);return;}
    if(ps==="client"&&pc&&isExpired(pc)){alert("Non puoi inserire "+pc+" in questa data: il contratto e' scaduto il "+clientEndDates[pc]);return;}
  ...
  var expAm=as==="client"&&ac&&isExpired(ac)?ac:null;
  var expPm=ps==="client"&&pc&&isExpired(pc)?pc:null;
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.45)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000}} onClick={onClose}>
    <div onClick={function(e){e.stopPropagation();}} style={{background:"#fff",borderRadius:16,padding:28,width:400,maxWidth:"92vw",maxHeight:"88vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,.2)",fontFamily:FONT}}>
      ...
      <HalfEditor label="Mattina" status={as} setStatus={sAs} client={ac} setClient={sAc} note={an} setNote={sAn} clients={clients} expiredClient={expAm}/>
      ...
      <HalfEditor label="Pomeriggio" status={ps} setStatus={sPs} client={pc} setClient={sPc} note={pn} setNote={sPn} clients={clients} expiredClient={expPm}/>
      <div style={{display:"flex",gap:10,marginTop:12}}>
        <button onClick={doSave} disabled={saving||expAm||expPm} style={Object.assign({},sB,{flex:1,padding:"11px 0",opacity:saving||expAm||expPm?.5:1})}>{saving?"Salvataggio...":(expAm||expPm)?"Contratto scaduto":"Salva"}</button>
