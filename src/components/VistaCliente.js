...
import { MESI, CL, FONT, sI, sO, fmtNum, calcMonthActuals, calcAllActuals, calcContractTotal, findClientFirstEntry, makeKey, daysInMonth } from "./shared";
...
export function VistaCliente(p){
  ...
  function detArrow(key){if(detSortKey!==key)return<span style={{marginLeft:3,fontSize:9,color:"#ccc"}}>⇅</span>;return<span style={{marginLeft:3,fontSize:9,color:CL.red}}>{detSortDir==="asc"?"▲":"▼"}</span>;}
  ...
  if(!sel){
    ...
    // Per ogni cliente cerchiamo la data della prima giornata in agenda
    // (findClientFirstEntry), poi calcoliamo "GG contratto" partendo da quel mese.
    // Se non esistono giornate, calcContractTotal ricade sul mese corrente.
    var rows=clients.map(function(c){
      var bud=cBud[c]||0;
      var ed=cEnd[c]||"";
      var fe=findClientFirstEntry(c,entries,cons);
      var ct=calcContractTotal(bud,ed,fe);
      var eTot=aAct[c]||0;
  ...
  // Dettaglio cliente: stesso principio, prima giornata in agenda come ancora del conteggio.
  var bud=cBud[sel]||0,ed=cEnd[sel]||"",feSel=findClientFirstEntry(sel,entries,cons),ct=calcContractTotal(bud,ed,feSel),eTot=aAct[sel]||0,act=mAct.byClient[sel]||0,rem=bud-act,cRem=ct-eTot;
  return(<div>
    ...
    {function(){var dayRows=[];
      ...
      return(<div style={{marginTop:20}}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10,flexWrap:"wrap"}}>
          <h4 style={{margin:0,color:CL.greyDk,fontSize:14}}>Dettaglio giornaliero</h4>
          {valCount>0&&<span style={{fontSize:12,color:"#2E7D32",fontWeight:600,background:"#E8F5E9",borderRadius:6,padding:"2px 8px"}}>{valCount} / {dayRows.length} validate ✓</span>}
        </div>
        <div style={{overflowX:"auto"}}><table style={{borderCollapse:"collapse",width:"100%",fontSize:13,fontFamily:FONT}}>
          <thead><tr style={{background:"#FFF8F8"}}>
            <th style={{padding:"8px",borderBottom:"2px solid "+CL.red,textAlign:"center",width:36,color:CL.greyMd,fontSize:11}}>Val.</th>
            <th onClick={function(){onDetSort("day");}} style={thDet}>Data{detArrow("day")}</th>
